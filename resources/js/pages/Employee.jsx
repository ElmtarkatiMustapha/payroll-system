...
import { employmentPeriods } from '../lib/payroll-calendar';
...
export default function Employee() {
    ...
    const employment = employmentPeriods(employee);
    ...
    return <><div className="no-print"><Link to="/employees" className="back-link"><ArrowLeft size={16} />{t('employees')}</Link>
        ...
        {employment.length > 1 && <section className="employment-history card"><div><h2>{t('employment_periods')}</h2><p>{t('employment_periods_hint')}</p></div><ol>{employment.map(p => <li key={p.id || p.start_date}><CalendarDays size={15} /><span>{date(p.start_date)} — {p.end_date ? date(p.end_date) : t('ongoing_employment')}</span></li>)}</ol></section>}
