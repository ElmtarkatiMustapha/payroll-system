...
export default function StatementSheet({ snapshot, statement, printable = false }) {
    ...
    return <article className={'statement-sheet print-area ' + (printable ? 'printable' : '')}>
        ...
        <div className="statement-meta"><span>{t('tshirt_size')}: <b>{employee.tshirt_size}</b></span><span>{t('trouser_size')}: <b>{employee.trouser_size}</b></span><span className="statement-daily-rate">{t('daily_salary')}: <b>{usedRates.map(fmt).join(' / ') || '—'}</b></span></div>
